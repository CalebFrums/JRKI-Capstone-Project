...
class UnemploymentModelTrainer:
        ...
        for target_col in self.target_columns:
            ...
            try:
                ...
                if len(train_series) < 20:  # Need minimum data for ARIMA
                    print(f"⚠️ Insufficient data for {region} ARIMA model")
                    continue
                ...
                print(f"✅ {region} ARIMA({best_order[0]},{best_order[1]},{best_order[2]}) - MAE: {mae:.3f}")
                
            except Exception as e:
                print(f"❌ ARIMA training failed for {region}: {e}")
    ...
    def prepare_lstm_sequences(self, data, target_col, sequence_length=12):
        ...
        for target_col in self.target_columns:
            ...
            try:
                ...
                if len(X_train_seq) < 20:  # Need minimum sequences for LSTM
                    print(f"⚠️ Insufficient sequence data for {region} LSTM model")
                    continue
                ...
                lstm_scalers[region] = {'X_scaler': scaler_X, 'y_scaler': scaler_y}
                ...
                print(f"✅ {region} LSTM - MAE: {val_mae:.3f}")
                
            except Exception as e:
                print(f"❌ LSTM training failed for {region}: {e}")
    ...
    def train_ensemble_models(self):
        ...
        for target_col in self.target_columns:
            ...
            try:
                ...
                if y_train.isna().sum() > len(y_train) * 0.5:  # Skip if >50% missing
                    print(f"⚠️ Too much missing data for {region} ensemble models")
                    continue
                ...
                print(f"✅ {region} RF MAE: {rf_mae:.3f}, GB MAE: {gb_mae:.3f}")
                
            except Exception as e:
                print(f"❌ Ensemble training failed for {region}: {e}")
        ...
        for target_col in self.target_columns:
            ...
            try:
                ...
                if y_test.isna().sum() > len(y_test) * 0.5:
                    print(f"⚠️ Insufficient test data for {region}")
                    continue
                ...
                print(f"✅ {region} test evaluation complete")
                
            except Exception as e:
                print(f"❌ Test evaluation failed for {region}: {e}")
    ...
    def generate_forecasts(self, forecast_periods=8):
        """Generate future unemployment forecasts for dashboard integration"""
        ...
        for target_col in self.target_columns:
            ...
            try:
                ...
                if region in self.trained_models.get('arima', {}):
                    ...
                    forecasts[region]['arima'] = arima_forecast.tolist()
                
                # Random Forest Forecasts (using last known values)
                if region in self.trained_models.get('random_forest', {}):
                    rf_model = self.trained_models['random_forest'][region]
                    # Use last row of test data as base for forecasting
                    last_features, _, feature_cols = self.prepare_features(self.test_data.tail(1), target_col)
                    rf_forecast = rf_model.predict(last_features)
                    # Simple approach: repeat last prediction
                    forecasts[region]['random_forest'] = [float(rf_forecast[0])] * forecast_periods
                
                # Gradient Boosting Forecasts
                if region in self.trained_models.get('gradient_boosting', {}):
                    gb_model = self.trained_models['gradient_boosting'][region]
                    last_features, _, _ = self.prepare_features(self.test_data.tail(1), target_col)
                    gb_forecast = gb_model.predict(last_features)
                    forecasts[region]['gradient_boosting'] = [float(gb_forecast[0])] * forecast_periods
                
                # LSTM Forecasts (if available)
                if (region in self.trained_models.get('lstm', {}) and 
                    TENSORFLOW_AVAILABLE and 
                    region in self.trained_models.get('lstm_scalers', {})):
                    
                    lstm_model = self.trained_models['lstm'][region]
                    lstm_scalers = self.trained_models['lstm_scalers'][region]
                    
                    # Use last 12 periods for LSTM prediction
                    last_sequence, _, _, _, _ = self.prepare_lstm_sequences(
                        self.test_data.tail(15), target_col, sequence_length=12
                    )
                    
                    if len(last_sequence) > 0:
                        lstm_pred = lstm_model.predict(last_sequence[-1:], verbose=0)
                        lstm_forecast = lstm_scalers['y_scaler'].inverse_transform(lstm_pred)
                        forecasts[region]['lstm'] = [float(lstm_forecast[0][0])] * forecast_periods
                
                print(f"✅ {region} forecasts generated")
                
            except Exception as e:
                print(f"❌ Forecast generation failed for {region}: {e}")
        
        # Save forecasts
        forecast_file = self.models_dir / "unemployment_forecasts.json"
        forecast_data = {
            'forecasts': forecasts,
    ...
    def run_full_training_pipeline(self):
        ...
        # Step 8: Generate Forecasts for Dashboard
...
def main():
    ...
    if success:
        ...
        print("\n❌ Model training encountered issues.")
